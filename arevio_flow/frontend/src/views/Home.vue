<template>
  <div class="home">
    ...
    <p><b-table
      ...
      :fields="fields"
      :filter="filter"
      :filter-included-fields="['company']"
    ></b-table></p>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      fields: [
        ...
        },
        {
          key: 'taxonomy',
          sortable: true,
          formatter: value => this.getFileNameFromPath(value),
        },
        {
          key: 'created_at',
          sortable: true,
  ...
  created() {
    if (!this.userprofile.companies.length) {
      this.options = [{ value: null, text: 'Please select a company'},];
      this.selected = null;
      return;
    }
    this.options = [];
    this.selected = this.userprofile.companies[0];
    this.userprofile.companies.forEach(item => {
      this.options.push({
        value: item,
        text: item.name,
      });
